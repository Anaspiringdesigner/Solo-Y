...
% (RL + Bridge are clipped images)
% =========================

% Pre-trainer (rounded rectangle, text only)
\node[
...
\node[font=\fontsize{9}{10}\selectfont] at (pretrainer.center) {Pre-trainer};

% Bridge (circle, clipped image)
\node[
...
% RL Environment (rounded rectangle, text only)
\node[
  draw=linec!60, line width=0.8pt, rounded corners=8pt,
  minimum width=1.75cm, minimum height=1.30cm, fill=none
] (rlenv) at ($(cA)+(3.75cm,-2.5cm)$) {};
\node[font=\fontsize{9}{10}\selectfont,align=center] at (rlenv.center) {RL\\Environ\\ment};

% TCN Encoder (bottom rounded rectangle, text only)
\node[
  draw=linec!60, line width=0.8pt, rounded corners=10pt,
  minimum width=2.75cm, minimum height=0.72cm, fill=none
] (tcn) at ($(cA)+(2.45cm,-3.95cm)$) {};
\node[font=\fontsize{11}{12}\selectfont] at (tcn.center) {TCN Encoder};

% ---------- Lines / arrows ----------
\draw[->,>=Stealth,line width=0.8pt,color=linec!70]
([xshift=-0.5   cm]pretrainer.south) -- ++(0,-0.12) |- (tcn.west);
...
% =========================================================
% Nodes + connectors INSIDE rectangle (rA,rB)
% Uses: images/do.jpg as default clipped image for 1..5 squares
% =========================================================
...
\node[font=\fontsize{10}{11}\selectfont] at (oscin.center) {OSCIN CHOP};
...
\node[font=\fontsize{10}{11}\selectfont] at (selchop.center) {select chop};

\draw[->,>=Stealth,line width=0.75pt,color=linec!70] (oscin.south) -- (selchop.north);

% -------- 1..5 squares (centered/lower, unique image each, no text labels) --------
\def\sq{0.66}
\def\gap{0.20}
...
% Put your 5 image paths here (left to right)
% Replace filenames below with your actual files for 1..5
...
\node[font=\fontsize{11}{12}\selectfont] at (selectblk.center) {Select};
...
\node[font=\fontsize{11}{12}\selectfont] at (spout.center) {Spout Out};
